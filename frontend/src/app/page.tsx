...
export default async function Home() {
  ...
  try {
    const res = await fetch("http://localhost:5000/api/produk", { cache: "no-store" });
    const data = await res.json();
              ...
              >
                ...
                <div className="relative overflow-hidden h-52">
                  <Image
                    ...
                    src={(imageUrl ? `http://localhost:5000${imageUrl}` : null) || src || "/images/placeholder.png"}
                  />
                  {/* Tag badge */}
            ...
            {galeriList.length > 0 ? galeriList.map(({ title, alt, img, src }, i) => (
              ...
              >
                <Image
                  ...
                  src={(img ? `http://localhost:5000${img}` : null) || src || "/images/placeholder.png"}
                />
              </div>
            )) : (
