...
export default async function Home() {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}/api/pengumuman`, { cache: "no-store" });
              ...
              >
                ...
                <div className="relative overflow-hidden h-52">
                  <Image
                    ...
                    src={(imageUrl ? `${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}${imageUrl}` : null) || src || "/images/placeholder.png"}
                  />
                  {/* Tag badge */}
            ...
            {galeriList.length > 0 ? galeriList.map(({ title, alt, img, src }, i) => (
              ...
              >
                <Image
                  ...
                  src={(img ? `${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}${img}` : null) || src || "/images/placeholder.png"}
                />
              </div>
            )) : (
