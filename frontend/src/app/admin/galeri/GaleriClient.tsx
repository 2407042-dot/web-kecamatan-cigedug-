...
const API_URL = `${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}/api/galeri`;
        ...
        {galeriList.map((item) => (
          <div key={item.id} className="border border-outline-variant/40 rounded-xl overflow-hidden shadow-sm flex flex-col">
            <div className={`relative w-full aspect-video bg-surface-container`}>
              <Image src={item.img.startsWith('/uploads') ? `${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}${item.img}` : item.img} alt={item.title} fill className="object-cover" unoptimized />
            </div>
