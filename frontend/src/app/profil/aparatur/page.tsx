...
export default async function AparaturPage() {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}/api/aparatur`, { cache: "no-store" });
                    ...
                    >
                      ...
                      <div className="p-5">
                        ...
                        <div className={`w-14 h-14 ${col.bg} rounded-2xl flex items-center justify-center mb-4 shadow-md overflow-hidden relative`}>
                          {imageUrl ? (
                            <Image src={`${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}${imageUrl}`} alt={nama} fill className="object-cover" unoptimized />
                          ) : (
