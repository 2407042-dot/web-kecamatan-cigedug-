...
export default async function InovasiPage() {
  let inovasiList: any[] = [];
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}/api/inovasi`, { cache: "no-store" });
    if (res.ok) {
            ...
            {inovasiList.map((inovasi) => (
              <div key={inovasi.id} className="group bg-white dark:bg-surface-container-low rounded-3xl border border-outline-variant/40 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col">
                <div className="relative w-full aspect-video bg-surface-container overflow-hidden shrink-0">
                  {inovasi.imageUrl ? (
                    <Image
                      src={`${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || "https://web-kecamatan-cigedug-backend-five.vercel.app"}${inovasi.imageUrl}`}
                      alt={inovasi.title}
                      fill
